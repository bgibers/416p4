...
int Clock::getAvgFps() const { 
  int totalFrames = 0;
  unsigned int frameMax = Gamedata::getInstance().getXmlInt("maxFrames");
  ...
  if (getSeconds()>0)
  {
    ...
    if(interval.size() > frameMax)
    {
      interval.pop_back();
    }

    for (auto& fps : interval)
    {
      totalFrames = totalFrames + fps;
    }

     interval.push_front(getFps());
   
  }

  return (totalFrames/frameMax);
